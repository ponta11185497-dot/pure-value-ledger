...
- **週次自動更新(GitHub Actions)が完全に動作するようになった**
  - `.github/workflows/weekly-update.yml` をpush済み、毎週月・木9:00(JST)に自動実行(2026-09-22に週1→週2回化)
...
- **2026-09-22: 情報源(sources.json)を大幅見直し**
  - トレカ関連の情報が拾えていなかったため、ポケカ以外の主要タイトル(遊戯王・ワンピ・
    DB・ヴァイス)の公式サイト/まとめサイト/Xアカウントを追加、単発記事・汎用サイト等
    7件を削除(計39件に)。おもちゃ・ウイスキー・アパレル・スニーカーの情報源も追加
  - 週次更新プロンプトに「応募がオンラインで完結するものだけを対象とする」という
    全体方針を明記し、category分類基準にも具体例を追加
  - 情報源増加でWebSearchの調査量が増え、1回の実行が63ターンかかりmax-turns(60)を
    超過→実際は成功しているのに失敗表示になる事象が発生したため、max-turnsを100に拡大
  - 締切の早い抽選を逃しにくくするため、実行頻度を週1回(月曜)→週2回(月・木)に変更
